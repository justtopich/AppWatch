...
AppWatch может отслеживать используемое место на локальных дисках. Для каждого диска укажите два порога оставшегося свободного места:
...
Для каждого задания можно указать пользовательские скрипты которые будут выполняться при срабатывании событий. Для одного задания можно задать только один скрипт через параметр  `eventScript`, располагаться они должны в папке `scripts`. Так, например, можно удалять старые лог файлы при срабатывании события окончания места на диске:
